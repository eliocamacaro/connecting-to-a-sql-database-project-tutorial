...
connection_string = f"postgresql://{'gitpod'}:{'postgres'}@{'localhost'}/{'db_prueba'}"
motor = create_engine(connection_string)
motor.connect()

# 2) Execute the SQL sentences to create your tables using the SQLAlchemy's execute function
motor.execute("""CREATE TABLE publishers(
...
sql_query = "SELECT * FROM publishers"
df_authors = pd.read_sql(sql_query, motor)
print(df_authors)